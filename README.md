...
data/
  ...
  teukgi_index.json        # 특기 인덱스(494특기) — fetch_teukgi_index 산출 (gitignore, 36MB)
  cache/                   # microdata 원시 캐시 (gitignore)
...
| 전체 정확도 | **0.909** (≈1s/문항) |
> 미답변 2건은 자기일관성 게이트가 보수적으로 거부(LLM 3샘플 표현 분기) — 안전 우선의 정직성 비용. **위험질문 거부율 1.0**이 핵심.
...
실제 API 호출에는 `MMA_SERVICE_KEY`(data.go.kr 서비스키)가 필요하다.
키 확보 전까지는 `data/fixtures`의 합성 데이터로 엔진을 검증한다.
