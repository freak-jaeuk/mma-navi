...
## 부록 — 개발 과정 (다단계 · Codex 리뷰 루프)

작성→**Codex 코드 리뷰**→수정→재리뷰 루프로 개발. 단계별로 잡아낸 주요 실이슈:

- **Phase 0-1~0-4** 결정론 백분위(gap 버그 수정)·거부 게이트(한국어 `\b` 누락 수정)·특기 매칭
  (등급요건 무시 거짓충족 수정)·평가셋 dev/held-out 분리(튜닝 FP를 held-out으로 적발).
- **Phase 0-5~0-6** 통합 데모 — 인덱스 동시성 레이스(KeyError) 수정·단일 카드 재설계·거부=우회로·
  build_roadmap KeyError·unknown 특기 오표기(정직성) 수정.
- **Phase 1~1+** 실 RAG(bge-m3 검색·로컬 Qwen 생성)·특기 의미매칭('보안'→포병❌→정보보호병✅)·
  임베딩 캐시 지문 정합성·B2G 히트맵.
- **Phase 1++** 공공데이터 실연동 확장 — 접수현황 경쟁률·복무기관/소집/공석·진로센터 → 로드맵.
  경쟁률 오배정(모집단위 분리)·공석 caveat·기준일 노출 수정. **실사용 7종 확정**.

문서: [기능명세서](기능명세서.md) · [데이터 및 기존 수상작](데이터_및_기존수상작.md) · [기획서](기획서_병역길잡이.md)
