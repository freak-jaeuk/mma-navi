...
결과 화면은 캡처해 `docs/screenshots/`에 아래 파일명으로 저장하면 이 섹션에 노출된다:
`health_card.png`(건강 백분위 카드) · `teukgi_result.png`(특기 추천 결과) · `roadmap.png`(준비 로드맵) · `admin_insight.png`(담당자 인사이트). 즉시 확인은 §8 '바로 실행'.
